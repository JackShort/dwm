 ...
 * driven through handling X events. In contrast to other X clients, a window manager selects for SubstructureRedirectMask on the root window, to receive events about window (dis-)appearance. Only one X connection at a time is
...
enum { SchemeNorm, SchemeSel, SchemeNormMon, SchemeSelMon }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - sw - x) > bh) {
		if (m->sel) {
			...
			drw_setscheme(drw, scheme[m == selmon ? SchemeSelMon : SchemeNormMon]);
...
drawtab(Monitor *m) {
	...
	for(c = m->clients; c; c = c->next){
	  ...
	  w = m->tab_widths[i];
	  drw_setscheme(drw, scheme[(c == m->sel) ? SchemeSelMon : SchemeNormMon]);
...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		mw = m->nmaster
			? m->ww * (m->rmaster ? m->mfact : m->mfact)
			: 0;
	else
		mw = m->ww;
